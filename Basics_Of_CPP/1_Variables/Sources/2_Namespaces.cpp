...
/*
    - Namespaces in C++
        ...
        - If we have our own function that is called cout C++ cannot tell the difference between the cout function in iostream 
            and our own we use namespaces to tell C++ which function to use.
    ...
    - Standard Input Output using cin and cout
        ...
        - >> = extraction operator, input streams
            ...
            - cin extraction ignores whitespaces: spaces, tabs, and newlines are considered termination characters,
                but if there is whitespace before and after the data the compiler will ignore the whitespace and only take in the 
                data.
            - cin >> var_x; cin first writes the data from the keyboard into a buffer and then reads the buffer to store into the 
                variable var_x.
            ...
            If we entered "100 200" during the first cin the buffer will hold BOTH the numbers cin >> x will take the first 
            number, and cin >> y will take the second number from the buffer. The whitespace after the 100 terminates the first 
            cin for x, when the second cin is executed the compiler will read the buffer from where it left off and store the 
            second number in the variable y.
            ...
            If we only enter 10.5, x will be 10 and z will 0.5. 1 and 0 are numbers but when the decimal point is reached >> will
            stop reading the buffer and take 10 and assign it to the variable x, for z the buffer will continue at the decimal 
            and continue to read it until it is done. z = 0.5

            :
            If we enter the string "Hello" the compiler will try to assign it to the variable x, and z but will fail to do so 
            since x is an integer and z is a double. we will get unexpected results.
*/
